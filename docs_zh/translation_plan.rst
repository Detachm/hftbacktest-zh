...
* 翻译页面标题、章节标题、简短说明。
* 常用 Python API 的 docstring 已中文化，包括初始化入口、订单/状态对象、Recorder、Stats、metrics、数据校验、订单延迟和快照工具。
* Rust/PyO3 绑定中 ``BacktestAsset`` 常用配置方法的 doc 注释已中文化。
* 交易所数据转换工具中的原始文件格式、JSON/CSV 字段名、外部 API 名称保留英文。
...
状态：已完成第一轮。常用 API 说明已中文化；交易所数据转换工具的大段原始格式示例保留英文。
...
状态：已完成第一轮。术语表已扩充，核心页面、教程、API 页可完整构建。后续仍建议人工通读重点教程并对照实盘语境继续精校。
...
.. list-table::
   ...
   * - ``index.rst``
     - 0 / 1
     - 已完成
     - 首页已接入用户指南、教程、API 和维护页面。
   * - ``glossary.rst``
     - 0 / 5
     - 已完成第一轮
     - 已覆盖主要回测、高频交易、做市和 API 术语。
   * - ``data.rst``
     - 1
     - 已完成
     - 第一版已翻译并接入中文目录。
   ...
   * - 教程 notebooks
     - 3
     - 已完成
     - 当前仓库 21 个 notebook 已全部翻译并接入导航。
   * - API Reference
     - 4
     - 已完成第一轮
     - RST 页面和常用 API docstring 已中文化；原始数据格式示例保留英文。
   * - 全站校对
     - 5
     - 已完成第一轮
     - 已做术语统一、构建校验和重点 API 校对；后续可继续人工精读。
   * - 社区反馈
     - 6
     - 未开始
     - 完成核心页面后再开 issue。
